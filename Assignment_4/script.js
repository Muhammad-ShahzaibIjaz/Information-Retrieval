document.addEventListener("DOMContentLoaded", function () {
    ...
    queryInput.addEventListener("input", async function () {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/v4/suggestions?query=${encodeURIComponent(query)}`);
...
document.getElementById("search-form").addEventListener("submit", async function (event) {
    ...
    try {
        let searchUrl = 'http://localhost:5000/api/v4/search?query=' + encodeURIComponent(query);
