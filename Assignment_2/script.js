...
document.getElementById("search-form").addEventListener("submit", async function (event) {
    ...
    try {
        let searchUrl = 'http://localhost:5000/api/v2/search/fulltext?query=' + encodeURIComponent(query);

        // Update URL based on the selected filter
        if (selectedFilter === 'title') {
            searchUrl = `http://localhost:5000/api/v2/search/title?query=${encodeURIComponent(query)}`;
        } else if (selectedFilter === 'author') {
            searchUrl = `http://localhost:5000/api/v2/search/author?query=${encodeURIComponent(query)}`;
        }
