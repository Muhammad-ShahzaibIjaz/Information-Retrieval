...
document.getElementById("search-form").addEventListener("submit", async function (event) {
    ...
    try {
        let searchUrl = `http://localhost:5000/api/v8/search/content?query=${encodeURIComponent(query)}&threshold=${encodeURIComponent(threshold)}`;

        // Update URL based on the selected filter
        if (selectedFilter === 'title') {
            searchUrl = `http://localhost:5000/api/v8/search/title?query=${encodeURIComponent(query)}&threshold=${encodeURIComponent(threshold)}`;
        } else if (selectedFilter === 'author') {
            searchUrl = `http://localhost:5000/api/v8/search/author?query=${encodeURIComponent(query)}&threshold=${encodeURIComponent(threshold)}`;
        }
