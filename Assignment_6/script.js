...
document.addEventListener("DOMContentLoaded", function () {
    ...
    filterOptions.addEventListener('click', (e) => {
        if (e.target.tagName === 'LI') {
            ...
            filterOptions.classList.add('hidden'); // Hide dropdown after selection
        }
    });
    ...
    document.addEventListener("click", (e) => {
        ...
        if (e.target.classList.contains('docPreview')) {
            e.preventDefault(); // Prevent default anchor behavior (if necessary)
            openArticle(e.target.id); // Call the function to open the article
        }
    });
});
...
document.getElementById("search-form").addEventListener("submit", async function (event) {
    ...
    try {
        let searchUrl = 'http://localhost:5000/api/v6/search/content?query=' + encodeURIComponent(query);

        // Update URL based on the selected filter
        if (selectedFilter === 'title') {
            searchUrl = `http://localhost:5000/api/v6/search/title?query=${encodeURIComponent(query)}`;
        } else if (selectedFilter === 'author') {
            searchUrl = `http://localhost:5000/api/v6/search/author?query=${encodeURIComponent(query)}`;
        }
        ...
        if (results.length > 0) {
            results.forEach((result, index) => {
                ...
                resultCard.innerHTML = `
                    <h3 id="${result.file_path}" class="text-lg font-semibold text-black docPreview hover:cursor-pointer">
                        ${index + 1}. ${highlightTitle}
                    </h3>
